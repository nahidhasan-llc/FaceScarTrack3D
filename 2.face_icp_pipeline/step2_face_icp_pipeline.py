...
def discover_patients(root: Path) -> dict[str, list[Path]]:
    ...
    for pat_dir in sorted(scan_root.iterdir()):
        if not pat_dir.is_dir():
            continue
        # Prefer *_aligned_oriented.ply (pre-oriented) over *_aligned.ply
        # Use whichever exists — oriented takes priority
        all_aligned  = {f.stem.replace("_oriented", ""): f
                        for f in pat_dir.glob("*_oriented.ply")
                        if "merged" not in f.stem}
        all_fallback = {f.stem: f
                        for f in pat_dir.glob("*_aligned.ply")
                        if OUTPUT_SUFFIX not in f.stem
                        and "merged" not in f.stem
                        and "backup" not in f.stem}
        # Merge: oriented wins where available
        combined = {**all_fallback, **all_aligned}
        plys = sorted(
            [f for stem, f in combined.items() if parse_scan_name(f.name)],
            key=lambda f: f.name)
        if plys:
